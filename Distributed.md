...
Raft 使用投票过程来阻止 Candidate 赢得选举，除非其日志包含所有已提交的条目。Candidate 必须联系集群的大多数成员才能当选，这意味着每个提交的条目必须至少存在于其中一个服务器中。如果 Candidate 的日志至少与大多数人中的任何其它日志一样最新（最新的定义如下），那么它将保存所有提交的条目。

#### （2）提交以前任期中的条目

Leader 知道，一旦该条目存储在大多数服务器上，就会提交当前任期内的条目。如果 Leader 在提交前崩溃，未来的 Leader 将尝试完成复制该条目。然而，Leader 不能立即得出结论，认为前一个任期的条目一旦存储在大多数服务器上就被提交了。

![](Images/Distributed_Raft_EntriesCommit.png)

一个时间序列，显示了为什么 Leader 不能使用旧任期中的日志条目来确定已提交。

- (a)：S1 是 Leader，并部分复制 index-2 处的日志条目；
- (b)：在 S1 崩溃中，S5 以 S3、S4 和自己的选票当选为第 3 任期的 Leader，并接受日志 index-2 的不同条目；
- (c)：在 S5 崩溃中，S1 重新启动，被选为 Leader，并继续复制，此时 index-2 处的日志条目已在大多数服务器上复制，但尚未提交；
- (d)：如果 S1 崩溃，S5 可以被选为 Leader（由 S2、S3、S4 投票），并从第 3 任期开始用自己的条目覆盖条目。然而，如果 S1 在崩溃之前在大多数服务器上复制了当前任期的条目，如 (e) 所示，则该条目被提交（S5 不能赢得选举）。此时，日志中所有先前的条目也都已提交。

为了消除上面的问题，Raft 从不通过计数副本来提交以前任期中的日志条目。只有 Leader 当前任期内的日志条目才能通过计数副本来提交。一旦以这种方式提交了当前任期中的一个条目，那么由于日志匹配属性，所有先前的条目都将间接提交。在某些情况下，Leader 可以安全地得出结论，认为提交了一个旧的日志条目（例如，如果该条目存储在每台服务器上），但为了简单起见，Raft 采取了更保守的方法。

Raft 在承诺规则中增加了这种额外的复杂性，因为当 Leader 复制以前条款中的条目时，日志条目会保留其原始条目编号。在其它共识算法中，如果新的 Leader 重新复制以前任期中的条目，则必须使用新的任期编号。Raft 的方法使对日志条目进行推理变得更容易，因为它们在一段时间内和跨日志中保持相同的任期编号。此外，与其它算法相比，Raft 中的新 Leader 发送以前任期中的日志条目更少（其它算法必须发送冗余日志条目以重新编号，然后才提能交）。

#### （3）Follower 或 Candidate 崩溃
...
- 选举超时应该比 MTBF 少几个数量级，这样系统才能稳定地进行。当 Leader 崩溃时，系统将在大约选举超时期间不可用。
